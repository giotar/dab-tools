...
try {
    ...
    let launch_response = await dab_client.launchAppContent("youtube", "dQw4w9WgXcQ", ["&list=PLFuNbp0NQ1D9ZMiyspdMS2hLtliqk9hVn&index=4"]);
    ...
    for (let app of appArr){
        ...
        let key_response = await dab_client.pressKey('KEY_CUSTOM_SELECT');
        console.log(`press "enter": ${app.friendlyName}, response: ${JSON.stringify(key_response)}\n`);

        await sleep(3 * 1000);
        key_response = await dab_client.pressKey('KEY_CUSTOM_SELECT');
        console.log(`press "enter": ${app.friendlyName}, response: ${JSON.stringify(key_response)}\n`);
